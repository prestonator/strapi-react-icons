...
const HomePage = () => {
  ...
  const updateIconLibrary = async (id: string, isEnabled: boolean) => {
    await client.put(`/strapi-react-icons/iconlibrary/update/${id}`, {
      data: {
        isEnabled: isEnabled,
      },
    });
  ...
  const deleteIconLibrary = async (id: string) => {
    await client.del(`/strapi-react-icons/iconlibrary/delete/${id}`);
    setIconLibraries((current) =>
  ...
  const importDefaultIconLibraries = async () => {
    ...
    defaultData.forEach(async (entry) => {
      await client.post("/strapi-react-icons/iconlibrary/post", {
        data: entry,
      });
    });
